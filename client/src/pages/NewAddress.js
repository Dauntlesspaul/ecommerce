...
function NewAddress() {
  ...
  useEffect(()=>{
    const fetchData = async()=> {
      try{
      const response = await axiosInstance.get('/fetchusernames')
  ...
  const handleSubmit = async() =>{
    ...
    try{
      setSubmiting(true)
      await axiosInstance.post('/newaddress', formDataObject)
      setSubmiting(false)
      navigate(-1)
    }catch(error){
      navigate('/login')
        ...
        <div className="grid place-items-center">
          <div className="relative bg-layer-img bg-cover bg-overlay bg-bottom shadow-md shadow-gray-300 text-black text-xl font-bold w-full h-36 grid md:h-64 place-content-center">
            <div className="relative z-1 flex items-center justify-center h-full">
              <h1 className="text-white text-xl md:text-4xl">Add Address</h1>
            </div>
          </div>
         ...
         <div className='grid place-items-center'>
            ...
            <div className='w-full flex justify-center'>
              <button 
                ...
                {submiting ? <CircularIndeterminate/> : <span>ADD ADDRESS</span>}
              </button>
              </div>
            </div>
          </div>
