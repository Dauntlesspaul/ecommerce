...
function WomensCollection({data, toast}) {
 ...
 const addToCart = (item) => {
  ...
  return (
    ...
    {data.womencollections.map((selector, key)=>{
   return (
        ...
        <div className="2xl:w-64 2xl:h-72 xl:w-56 xl:h-64 lg:w-48 lg:h-56  w-40 h-48 bg-neutral-100 p-1 relative hover:p-1.5">
            ...
            <button className=" bg-neutral-100 hover:bg-neutral-200 w-full h-7 flex justify-center items-center" onClick={() => addToCart({ brand: selector.brand, units:selector.units , rating: selector.rating, discountprice: selector.discountprice, imgurl: selector.imageurl, size: [6, 6.5, 7, 7.5, 8, 9, 10, 11, 12, 13], sizeIdex: 0})} ><Cartsvg width="w-5"/><span className='text-sm font-semibold text-red-600 ml-1'>Add to cart</span></button>
        </div>
